...
- ⚠️ **§5「露一半」两个数据态都不合规，根因在设计稿：**
  ...
  **根因是量出来的**：把稿子 `data-screen="training"` 那两段 `.note`（共 **41px**）`hidden` 掉重跑
  ...
  修它要往那条带里放**真东西**：43–63px 能让两个数据态同时落进 [19,57]（两态相差 18，窗口宽 39）。
  规范 §5 给训练营留的正是**每日一题**那一块（见上面第 ⑧ 条）。**放什么是产品决定，交 Fan。**
  闸已经写下（`kiosk-shell-scroll.spec.ts`，标 `test.fail()`）：现在必须红，**修好了会反过来变红**，
  提醒把标记删掉。⚠️ `test.fail()` 写在文件作用域会套住后面每一条 —— 实测把五条真闸一起
  标成「预期失败」，整份文件只剩这一条「过」。必须写在用例体里。
