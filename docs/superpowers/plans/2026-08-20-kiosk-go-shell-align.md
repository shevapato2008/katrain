...
> **⚠️ 2026-09-16 最新更正覆盖上一条：** 对照星阵机器人界面后，Fan 确认悔棋应**保留为灰色**；
> 机器人走无状态 genmove 隧道，没有可调用的悔棋协议，因此这颗键整局禁用且不随算招状态重排。
> 同轮补齐停一手（实测 `-1`）、机器人认输（实测 `-3`）与免费 `judge` 数子。
...
| 旁注说「升降级局下完也不给复盘」 | **没有任何实现**：`POST /reports/` 不看 game_type。真正的那道闸是 `interface.py` 的 `ANALYSIS_ACTIONS`（`analyze_all` / `analyze_current` / `show_pv` …），关的是**对局中那棵树**上的动作 —— `analyze_all` 是「把当前这局从头算一遍」，不是离线报告。国象稿子明写「两者进的是同一条复盘线、同一份报告」 | 按国象和后端来。**根子在共享规范** `kiosk-shell-spec.md:895` 把 `analyze_all` 读成了「复盘」，误读顺着规范流到围棋稿又流到本计划 —— **三处已全部更正**（2026-08-23） |

⚠️ **计划正文里「现状列表里如果出现了 ranked 局，要确认它不给『开始分析』」那句作废。**
计分局下完了照样该有报告。**真正该挡的是没下完的局** ——
判别位是「这局结束了没有」，**不是「算不算分」**：拿离线 KataGo 把残局算完再回去接着下，
那才是真作弊通道。用 `isRated` 去管它，就又变成一个 prop 兼管两件事、逼调用方撒谎。

### 2026-08-23 复核后补的两处（裁定 agent 提的，核实属实）

1. **「查看报告」在两档都跑完时没有唯一宾语。** `taskId` 是**按档**发的，同一局可以同时挂
   标准和精读两份完成报告。上一版的 `ReportGameCard` 本来就是两个键，我收成一个 = 丢了一条路。
   ⇒ `rowState` 现在带 `taskIds`（两档都在里面），行尾据此决定画一个「查看报告」还是
   「标准」「精读」两个。真浏览器加了一条闸：**两档都跑完 + 这一行还选中**时，
   行尾是四个元素（标 + 两键 + 删除），量它装不装得下。
2. **「一行上不许有三个可点的东西」这条理由自相矛盾** —— 删除挂到选中行之后那一行仍然是三个。
   准确的说法是**未选中的行上不放不可逆操作**：那些行你只是扫过去，而选中的那一行是你刚点过的。
   注释已改成后者。
...
### Task 19 修订（2026-08-23，做完之后回填）

**这一 Task 没写一行产品代码** —— 五屏的共享外壳在 Task 1–8 就接好了，
这一步做的是**把它钉住**：`kiosk-shell-geometry.spec.ts` 加了五条（一屏一条），
量顶栏 `1024×56@(0,0)`、内容区 `x16 / w992`、无 Dock 时下缘贴 600。

⚠️ 写第一版时把内容区上缘写成了 `topbar.h + 14` —— **量的是外框不是内沿**。
规范 §5 说的「内容从 y70 起」说的是内沿，外框在 56，差的正好是 `--content-pad-y` 的 14。
两套数不矛盾，但混着读会以为对不上；同文件上面那条 L1 的闸早就把两套都钉过一次了。

### Step 5 那条「眼睛看不跳」用不着单独做

它要的是「从 `/kiosk/play` 逐个切过去，顶栏和中间区外框不许动一个像素」。
而上面五条量的是**绝对值**（0,0,1024,56 / x16 / w992），`/kiosk/play` 那条 L1 闸量的是同一批数 ——
**五个绝对值相等就是不跳**。再写一条「切过去再量一次」是同一件事量两遍。

### 仍然登记（本轮不动）

- `ResearchPage` 那三条手写的 52 高面包屑条：864 行、四个渲染分支、三处结构各不相同，
  动它超出「接壳」。**这是第三次登记**（Task 8 / Task 19 / 这里）。
- 摆谱和直播**判成 L2**：Task 4 把它们下了 Dock（规范 §3 只许一个棋种专属项），
  于是 `dockLevelOf` 判 2、不出 Dock、内容区 516 高。**这是那条裁定的后果，不是 bug。**
  计划 Step 2 说「`BaipuListPage` 接 `KioskConsoleRail`」—— 接不了，`showRail` 要求 `level === 1`。

---

## Task 21（计划外，2026-08-23 第二轮）: 屏 02 / 03 开局设置

计划的任务表按**十屏稿**编的，开局设置三屏一屏都不在里面；而 D2 当时把摆谱 / 直播 /
研究 / 跨平台 / 标定判成「稿子没画」。**这两条前提在 8-21 那轮稿子扩到 27 屏时就已经
不成立了** —— `sample-go/shots/` 里 27 张参考图一张不缺。剩下的每一屏都有参照物。

屏 02 / 03 是**同一个组件**（`pages/AiSetupPage.tsx`，`:mode` 分岔）。右栏从
「两列 MUI 下拉 + `overflow:hidden`」整块换成稿子的 `.setgrp` 体系；新构件三个
（`KioskOptSeg` / `KioskStepTrack` / `handicapStones()`），`OptionChips` 改成拼装。

**四处按实现纠正稿子**（落子不是设置项 / 我执两项不是三项 / 赌注按净胜分说话 /
AI 策略只有一条说明有出处）、**两个四图逮到的真 bug**（让子局的盘没摆子、净胜分负号写成 `+-1`）、
**一处我自己编的**（给升降级编了一段 `.setnote`，稿子那一屏根本没有）。
承重反查触发并量过（这一屏是这一轮才第一次能滚的），变异做了三次、第三次才红 ——
前两次证明这条链是共享外壳在扛。

屏 03 的「对手」那一格**没换壳**：`AiLadderSetupOpponent` 手上有六种诚实状态而稿子只画了一种，
且它是 galaxy 那屏的共同消费者。登记。

全部经过与实测数字见 `superpowers/tracks/kiosk-go-shell-align/scope.md` §11 / §12。

---


## Task 22（计划外，2026-08-23 第三轮）: 屏 04 本地对局 · 开局设置

和屏 02/03 同一副骨架，**但是另一个页面组件**（`pages/PvpLocalSetupPage.tsx`，
上一版根本没接过外壳：`LiveBoard` + 两个 `TextField` + 四条 `Slider` + 两个 `Switch`）。
新构件零个 —— 屏 02/03 那一轮建的全能复用；新写的只有 `.nameinput` 一条 CSS 和
`utils/setupOptions.ts`（用时七档 + 规则四条说明，两屏共用的**契约**，不是「看着通用」）。

**稿子这一屏四处不成立**，两处是这一轮才查出来的产品事实：

- **「白方 · 贴目的一方」反了** —— 贴目是黑方贴给白方的（`core/game.py:372`）。
- **`.setnote` 两句都得改**：「没有形势判断」不成立（`pvp_local` 不在
  `SCORING_GAME_TYPES` 里 ⇒ 对局屏那颗「领地」照样能按；真正关掉的是胜负走势图和 AI 支招），
  「段位只有在线大厅的定级队列会改」也不成立（定级赛在**升降级对弈**，
  `RANK_MOVING_GAME_TYPES` 只有一项）。**这类文案最容易编，因为它说的是别的屏上的事。**

**「点此输入」在实现里是真输入框**，不是药丸 —— 静态稿画不出输入，那不是稿子的错。
留空送空串、后端因此不写 `PB`/`PW`，**前端不替用户编名字**（编出来的会进棋谱）。

承重那条闸**参数化成两屏各跑一次**：共用 CSS 但骨架各自手写，量一屏推不出另一屏。
变异（把屏 04 的主行动键搬进滚动区）让屏 04 那条红、屏 02 那条绿 —— 证明它量的是屏 04 自己。

全部经过与实测数字见 `scope.md` §14。

---


## Task 23（2026-08-23，Fan 一句话问回来的）: 「落子」那颗开关接成真的

Fan 看屏 04 四图时问「为什么最终实现都没有屏幕选项」。**我把稿子判错了。**

屏 02 我写的判据是「`isVisionEnabled` 由后端给,全仓没有任何地方能让用户切它」——
后半句是错的:做题屏(`TsumegoProblemPage.tsx:626`)早就有这颗开关,而且正是稿子画的
那个语义。错误的形状是**「我没找到」被写成了「不存在」**:我 grep 的是 `isVisionEnabled`
的消费者,而那颗开关是**另一个位**,根本不叫这个名字。

真实形状是三段(设备能不能 / 这一局想不想 / 实际落在哪),第一版把中间那段抹掉了。
`.igfix` 读数**不是假话**,但判据错了、结论碰巧不假 —— 这比结论假更难发现。

接法三处,缺一个就是半截开关:`utils/playInput.ts`(偏好,和做题屏**两把键**,
因为默认值必须相反)、`components/vision/PlayInputGuard.tsx`(对局四条路由外面,
选了屏幕就不再被标定守卫挡住)、`GamePage`(14 处 `isVisionEnabled` → `physicalPlay`)。

顺带又犯了一次上一轮那个错:说明行做成常驻,而**稿子 03 屏那一组没有说明行** ——
四图 `refOnly` 当场从 10283 涨到 12152,改成条件渲染后降到 10052。

三屏九个四图计数**没有一个变差**;屏 05 重跑 87/614400 像素、最大通道差 5 ⇒
默认路径**纯增量**。全部经过见 `scope.md` §15。
...
## Task 20 修订（2026-08-23，做完之后回填）

### Step 3「重跑四图零字节变化」**前提不成立**

重跑一遍，27 个 PNG 全变。查下去不是非确定性 bug，是**抗锯齿抖动**：
`01-play--implementation.png` 新旧逐像素比 —— **135 / 614,400 个像素（0.02%）**，
最大通道差 **9/255**，包围盒 `[28,10]–[998,580]`（**散落全图**，不是某一块内容变了）。

⇒ 「零字节」这个判据用不了。能用的两条替代都比它强：
1. 四图闸每次跑都打印 `both / refOnly / implOnly` —— **内容变了这三个数会变一个量级**，
   而抖动只让它们动几百（实测 15-kifu 抖 258，占 0.8%）。
2. 承重和契约那两组闸本来就是逐像素量真浏览器的 —— 它们才是「实现和代码同步」的证明。

那 27 个 PNG 的改动**已撤回**：它们不携带任何信息。

### Step 1 白名单没清空，改成**按理由分了三类**

31 条剩下的都注明了为什么允许，三类：
**(A)** D2 稿外五屏的内容区（没有参照物就没有四图闸，重画等于自己发明设计）；
**(B)** 稿子画了、但本轮十个 Task 没排到的屏（开局设置三屏 / 在线大厅 / 训练营分类 /
课程书目 / 课程小节）；**(C)** 对话框与浮层（不在 1024×600 的版式里）。
**三类之外一个都不许有** —— 新增一条 = 有人在已经重画过的屏上又引了 MUI 图标。

### Step 2 全量基线

- `npx playwright test --config playwright.visual.config.ts`：**124 条全绿，零失败**
  （12.1 分钟，workers=2）。
  ⚠️ 计划里记的「21 红 / 137 绿」是**另一套跑法**下的数（默认 config 会起 Python 后端），
  两组数不可比。本轮凡是依赖后端的地方都已经在闸里 stub 掉了 ——
  **后端起着就绿、一停就红的东西不叫闸**（做题屏那两条几何闸原来就是这样）。
- `npx vitest run`：**1360 绿 / 1 红**，那一条红是既有的
  `GamePageEngine.test.tsx › confirming resign…`（动手前就红）。

### Step 6 上板走查**没做** —— 要板子

RK3562 那三条（冷启动看左栏盘、字体是不是霞鹜文楷 + 龙藏、2G 内存一次只跑一家）
都必须在真机上量。**这里不写「看着应该没问题」** —— 那是把没查过说成查过了。
