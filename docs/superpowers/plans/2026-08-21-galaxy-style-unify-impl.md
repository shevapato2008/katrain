...
## 4.14 S9 收口（commit 6326211c + 3675112a）

### 折叠最后一份工具格实现

`ResearchToolbar` 的本地 `ToolButton` 是第四份也是最后一份，形状是「挂 onClick 的
`<div>` + 没有可及名」—— 键盘到不了、读屏念不出、**控件账本整整数不到 12 个键**。
折进 `ToolGridButton` 后八个开关补 `toggle`（报告 `aria-pressed`），四个一次性动作不报；
图标从 22 统一到 18。kiosk 有自己那份 `kiosk/components/research/ResearchToolbar.tsx`，
不受影响。

### 坐标默认接上 spec §3.2

`useBoardCoordinates`（`visible = userOverride ?? edge >= 500`）早就写好也有测试，
只是这两页没接。`ResearchPage` 原来有**三个各说各话的来源**：初值里的 `coords: true`
（共享 `Board.tsx:194` 真的读它 —— 我一开始判它是死字面量，查过才知道不是，
**否定的结论必须去被读的那一侧确认**）和两处写死的坐标开。现在只剩一个来源。

真浏览器实测（jsdom 没有布局引擎，这一条它无权作证）：
...
中间三档是**判别档**：视口都 <900（已堆叠）。若闸错量成视口宽度，560 那档会关。
翻转发生在边长 500 上而不是视口 900 上 —— 闸量的是 spec 说的那个操作数。
（同族：[[reference_gate_measures_wrong_operand]]。）

### 全量控件账本 diff（19 条路由，对 fd3de2b6 现跑基线）
...
三条路由控件**变少**，合计 −6：死活题分类/单元/列表各 −1 / −2 / −3 —— 正是 S8 删掉的
六个面包屑跳转链接，与 S8 报的数吻合。研究页 **+12** 正好等于本步折叠的 12 个工具键
（折叠前它们是 div，选择器根本选不到，所以「账本数不到」不是修辞）。

**未覆盖的三条路由**（如实记，不静默截断）：`GameRoomPage`（要多人对局会话）、
`LiveMatchPage`（要直播数据，本机没有）、升降级态的 `GamePage`（要走完整升降级流程）。
它们各自的账本在 S5 / S1 已单独做过并经确认；这一轮的全量 diff 是**回归检查**，
不替代那几步。

### 账本这个闸自己错了三次

工具错三次、产品零次，值得单列：
1. **basename 配对**：eslint 基线用同目录 `__base_*.tsx` 副本对账，第一版按文件名配对，
   把没碰过的文件全算成新增（报出 41 个假新增）。改按完整路径配对后是零新增。
   （[[reference_new_failures_only_from_baseline_diff]]）
2. **把 `disabled` 当影子**：第一版把「不可聚焦」等同于「不是真控件」，于是禁用态的
   工具键被整体漏掉。判据改成只排除 `aria-hidden` / `tabindex<0 且未禁用` / 零尺寸。
3. **可及名只查祖先 `<label>`**：MUI 的 `TextField` 把 `InputLabel` 渲染成**兄弟**并用
   `for=id` 关联，于是研究页的贴目框被误报成无名。补上 `aria-labelledby` 与 `label[for]`
   之后归零 —— 那是**误报，不是缺陷**。

另有一条环境性误判：报告详情页最初取的 task id 不属于探针用户，页面停在 404 错误态，
账本读到 5 个无名禁用工具键。换成有效数据后是 14 控件 / 0 无名。
**但错误态本身是真状态**：404 下那 5 个键确实没有可及名（标签取自没加载到的数据）。
不在本 track 范围，记入 §5 待议。
...
|---|---|---|
| `tsc -b` | — | 0 |
| eslint 全仓 error | 257 | **256**（净少 1，零新增） |
| eslint 边界规则 | — | 0 违规 |
| `vitest run` | 同一条 kiosk 用例红 | 1248 passed，**同一条**红 |
| kiosk dist | 13,340,826 B / 213 文件 | 13,341,487 B / 213 文件（**+661 B**） |
| 全量 dist | 31,396,131 B / 277 文件 | 31,392,038 B / 277 文件（**−4,093 B**） |

kiosk 体积按计划要求**对同一 commit 上现跑的一次构建**，不拿磁盘上现成的 dist 当基线：
先 `git checkout fd3de2b6` 现跑一次再比。文件数两侧都是 213 —— 没有新 chunk 泄进 kiosk。
先前用 `du -sk` 量得两侧完全相同，那是磁盘块取整盖住了 661 字节；**按块量的体积闸
分辨不出小于一个块的回归**，要用字节。

`GamePageEngine` 那条 kiosk 用例在 `fd3de2b6` 上单独跑也是红的 —— 已确认与本 track 无关，
不是「看着不相关就归给既有噪声」。

## 5. 待议（发现即记，不顺手改）

> **2026-08-22 收口**：Fan 要求把本节剩下的六项逐一解决。六项都已完成，
> 分别在 commit `8a5a028b` / `062b19c4` / `eeb867ee` / `b6f9969a` / `c99ac382` / `8604fbdd`。
> 每一条的结论就地标在下面对应条目上。**其中两条的记载是错的**：
> ①的根因不是「标签取自没加载到的数据」，④的「本机没有直播数据」并不成立。
> 两条都是**按当时的推断写的、没有去查**，这是本节最该记住的一点。
...
- ~~**直播页的显示开关可能也在折线以下**~~ **已解决（2026-08-22，commit b6f9969a）**：
  那条待议的前提是错的 —— 本机库里有 **123 场真实对局**，最长 357 手，其中 `yike_182849`
  有 312 手全部分析完、48 处失误。实测：1440x900 折线下 **32px**、1024x768 下 **164px**，
  与复盘页同一形状，已用同一改法（挪进 railBody、紧跟对局信息）修好，三档全绿。
  **一处险些放过的取数错误**：第一次用手数最长的 `xingzhen_185099` 量，1440 档是**过**的 ——
  它那 358 行 `live_analysis` 全是 pending，AI 推荐面板停在空态只有 115px。
  「手数最多」不等于「右栏最长」，判据得挑**分析真的做完**的那一场。
  原始记录：复盘页实测工具格掉到折线下 208–389px，原因是真数据把
...
- ~~**「进入研究室」是个空跳转**~~ **已补（Fan 2026-08-22 点头「补」）**：
  ...
  3. **SGF 用刚取回的 `detail.sgf_content`，不从 `board` 反推** —— `loadFromSGF` 的
     ...
     **已解决（2026-08-22，commit eeb867ee）**：真浏览器实测（真实棋谱 id=4，181 手）改前
     **没有 `POST /api/sgf/load`**，会话建成空棋盘、扫的也是空棋盘；改后 sgf/load 11441B、
     界面「已完成 0 / 182 步」。根因不是「等得不够久」—— `setTimeout` 捕获的是 effect 那一帧的
     `handleStartAnalysis`，等 1000ms 一样错。改成「立标志 + 独立 effect」，
     顺带把同一闭包里同样读旧值的 `getSnapshot()` 和 `initialMove` 一起修好。
...
- **规范 §3.2 的坐标默认档，研究页与死活题页没做到**：spec §3.2 写「棋盘边长低于 500px 时
  ...
  **✅ 2026-08-22 已在 S9 做完（commit 6326211c）**，Fan 点头。实际比预估多一处：
  `ResearchPage` 不是「一行」而是**三个真相来源**（初值里的 `coords: true` 被共享
  `Board.tsx:194` 真的读，加两处写死的坐标开），三个 shell 都要接 `onBoardSizeChange`。
  真浏览器五档实测见 §4.14，含三档「视口 <900 但棋盘 ≥500」的判别档。
...
- ~~**报告详情页在 404 错误态下渲染 5 个无名禁用工具键**~~ **已解决（2026-08-22，commit 8a5a028b）**。
  根因与这里记的**不一样**：不是「标签取自没加载到的数据」，而是 `LoadingControls`/`LoadingActions`
  本身就是 `<Button disabled><Skeleton/></Button>`，按构造就没有可及名；而且同一份实现
  在直播观战页（原件）是 6 个、两页的**加载态**也这样渲染，实际共 11 个。
  改法：占位不再做成控件；未登录态和错误态不再挂 `displayControls`/`actions`（它们不是加载态）。
  原始记录：（2026-08-22，S9 全量账本时量到）。
  正常数据下这一页是 14 控件 / 0 无名；但 task 不存在时页面进错误态，那 5 个工具格按钮
  的标签取自没加载到的数据，于是既没有可见文字也没有 `aria-label`。**错误态是真状态**，
  读屏用户在那一屏上会听到五个没有名字的按钮。改法是给这些键的 label 提供与数据无关的
  兜底文案。不在全站风格统一范围内。

- ~~**复盘页在 430 档根本不堆叠**~~ **已解决（2026-08-22，commit 062b19c4）**：
  基态改成窄档、`@media (min-width:900px)` 才并排，与 `BoardPageShell` 同一组断点。
  430/899 堆叠、900/1024/1440 并排，**1440 档改前改后逐数相同**（宽档零回归是量出来的）。
  判据脚本 `loadbearing_reports_page.js` 已入仓。原始记录：（2026-08-22，S8 承重实测时逐层反查出来）。
  它的两栏是 `flex-direction: row; flex-wrap: nowrap`，窄屏不换行也不堆叠，左栏被压到
  **48px 宽**、右栏 520px，**两栏合计 568 已超出 430 视口**，左栏那句提示文字因此竖排成
  595px 高。那几行布局代码本 track 一个字没动 —— **既有缺陷**，记账不顺手改。

- ~~**`proto/check.py` 不与 spec 对账**~~ **已解决（2026-08-22，commit c99ac382），但口径改了**：
  冻结原型**只活在 /tmp 的 scratchpad 里、没有 git**，在那儿加闸等于给一个下次会话就消失的
  东西加闸。按「闸建在操作数所在的那个仓」：归档原型源码入仓（744KB），并新增
  `check_spec_sync.py` —— 规范小节正文的哈希与各消费方的声明对账，改了正文没跟 → 红，
  **改了正文忘了 bump 版本号 → 同样红**（判据是哈希不是版本字符串）。原始记录：（2026-08-22，S8 发现）。它只检查冻结原型内部自洽，
  所以「规范改了、稿子没跟」这类漂移它是全盲的 —— 内容页页头的 `cph()` 就这样带着
  裁定前的形状活到了 S8，靠人工比对才抓到。同族：[[project_kiosk_go_design_27_screens]]
  记的「规范早写了但没闸、稿子带着三处过了半年」。**改法**：给 check.py 加一条
  「稿子里每个 `data-zone` 的形状要能对上 spec 的哪一节」的对账，或至少在 spec 修订时
  强制回头扫一遍原型。不在本 track 范围。

- ~~**web 服务清理会话时把自己挂住**~~ **已解决（2026-08-22，commit 8604fbdd）**。
  本机一小时内两次「进程还在、端口还听着、对所有请求不回话」。`sample <pid>` 采到的栈是
  决定性的：主线程（uvloop 事件循环）停在 `lock_PyThread_acquire_lock`，25 个线程里 18 个
  栈上都有同一把锁，**没有一个停在阻塞 `read()` 上**，另有一个 KataGo 僵尸子进程。
  链路是三段叠起来的，**少改任何一处，另外两处都还能把服务挂住**：
  `_cleanup_loop` 跑在事件循环上 → `cleanup_expired()` 持 `SessionManager._lock`
  → `KataGoEngine.shutdown()` 里三个 `t.join()` 没有超时。三处各修一处，
  闸在 `tests/web/test_session_cleanup_does_not_block.py`（四条，变异逐条实跑）。
  回归判据用**基线 diff**（`engine.py` 是桌面版和 kiosk 共用的）：改前改后各跑一次全量
  pytest，失败**名字集合**逐条相同（各 119 条，`comm` 两个方向都空）。
  上一轮我给这条排过三种可能性、把「换页慢」排第一 —— 那是按代码猜的，栈把它排除了。

- **全量 pytest 往本机真实 dev 库里建用户**（2026-08-22 收口时量到，**记账不改**）。
  为了给⑥做基线 diff，全量 `pytest tests` 跑了两次，之后本机 `katrain_db` 多出
  **57 个 `alice-*/bob-*/mallory-*` 用户** —— 库里此前已经积了 500 多个同样式的，
  说明这是测试套件一直以来的行为，不是这次引入的。它违反「Fixture 必须隔离，
  并在创建时就写明删除条件」：这些行既没有隔离（直接落生产 schema 的 `users`），
  也没有删除条件（跑完就留着）。没有顺手删 —— 删掉这 57 个而留下那 500 个只会
  更难分辨，而且下次跑还会再长。要修得在测试夹具层做（事务回滚或独立 schema）。

- **`create_session` 在锁内构造引擎**（2026-08-22 修上一条时顺手看到，**记账不改**）。
  `WebKaTrain(enable_engine=True)` 会拉起一个 KataGo 子进程，而它是在 `self._lock`
  里面构造的 —— 与上一条同一个「慢活儿在锁内」的形状，只是这次没抓到现行。
  动它要重排名额检查的竞争语义，不在这次范围内。
