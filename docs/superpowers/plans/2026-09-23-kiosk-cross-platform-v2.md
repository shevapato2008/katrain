...
撤掉的两样：**路数那一行 igrow**（降成提示行里的半句）、**整个「这一局会是」段**（贴目进组标题 secval，规则/路数进提示行，「胜负只进星阵那边的账」页控条副标已经在说）。

> ⚠️ **「不计时」不进这一屏** —— 这不是漏写。核过：Fan 审核通过的设计稿里屏 09 **从头到尾没提过时间**（那一屏的标记里「计时/时间/不限时/分钟/读秒」全部零命中）。用户在屏 10 拿得到（钟位写「第 N 手 / 不限时」）。
> 另有一条硬约束：`.kiosk-opthint` 是**定高**（`tokens.css:720-722`，`height` 与 `line-height` 同值），往里加字一旦折行会被**静默裁掉**，而现串已接近 460 栏宽的边界。
> 这一条在 Task 4 的视觉关卡上作为一行问题交给 Fan，不在实现里自作主张。
