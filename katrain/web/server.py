...
async def _lifespan_server(app: FastAPI, log):
    ...
    if settings.PREVIEW_MODE:
        ...
        # 与上面的 reconcile 同处 PREVIEW_MODE 守卫之内：这三件都是**往库里写钱**
        # （结算、回收孤儿预扣、补开账账本行），而 preview 与生产共用同一个 postgres
        # 与同一个库名。git 这次是无冲突自动合并的，缩进保持在守卫之外 —— 那是
        # 「合得干净」不等于「合得对」的典型：这里是人工挪进来的。
