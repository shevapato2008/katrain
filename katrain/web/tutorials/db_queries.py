...
def get_book_counts_by_category(db: Session) -> Dict[str, int]:
    ...
    rows = (
        ...
        .group_by(TutorialBook.category)
        .all()
    )
...
def get_books_by_category(db: Session, category: str) -> List[TutorialBook]:
    return (
        db.query(TutorialBook)
        .options(selectinload(TutorialBook.chapters))
        .filter_by(category=category)
        .order_by(TutorialBook.title)
        .all()
    )
...
def get_book(db: Session, book_id: int) -> Optional[TutorialBook]:
    return (
        db.query(TutorialBook)
        .options(joinedload(TutorialBook.chapters).joinedload(TutorialChapter.sections))
...
def get_chapters_by_book(db: Session, book_id: int) -> List[TutorialChapter]:
    return (
        db.query(TutorialChapter)
        .options(selectinload(TutorialChapter.sections))
...
def get_sections_by_chapter(db: Session, chapter_id: int) -> List[TutorialSection]:
    return (
        db.query(TutorialSection)
        .options(selectinload(TutorialSection.figures))
...
def get_section(db: Session, section_id: int) -> Optional[TutorialSection]:
    return db.query(TutorialSection).options(joinedload(TutorialSection.figures)).filter_by(id=section_id).first()
...
def update_figure_narration(
    db: Session, figure: TutorialFigure, narration: str, audio_asset: str | None
) -> TutorialFigure:
...
def record_payload_history(
    db: Session, figure_id: int, board_payload: dict, changed_by: str, change_type: str = "edit"
):
