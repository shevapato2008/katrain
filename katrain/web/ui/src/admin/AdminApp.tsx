...
import { ArrowLeft, BookOpen, Camera, ChevronLeft, ChevronRight, Clock3 } from 'lucide-react';
...
export default function AdminApp() {
  ...
  const [page, setPage] = useState<'tutorial' | 'cron' | 'vision'>('tutorial');
  ...
  return <div className={`admin-app ${page === 'tutorial' ? 'admin-app-tutorial' : 'admin-app-cron'}`}>{header}<div className="admin-wrap"><aside className="admin-side" aria-label="管理导航"><div className="admin-sidehead">内容与服务</div><button type="button" className={`admin-nav ${page === 'tutorial' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', cursor: 'pointer', border: 0, fontFamily: 'inherit', background: page === 'tutorial' ? 'var(--admin-active)' : 'transparent' }} onClick={() => setPage('tutorial')}><BookOpen aria-hidden="true" />教程管理</button><button type="button" className={`admin-nav ${page === 'cron' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', cursor: 'pointer', border: 0, fontFamily: 'inherit', background: page === 'cron' ? 'var(--admin-active)' : 'transparent' }} onClick={() => setPage('cron')}><Clock3 aria-hidden="true" />定时任务</button><button type="button" className={`admin-nav ${page === 'vision' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', cursor: 'pointer', border: 0, fontFamily: 'inherit', background: page === 'vision' ? 'var(--admin-active)' : 'transparent' }} onClick={() => setPage('vision')}><Camera aria-hidden="true" />视觉实验室</button><div className="admin-sidefoot">当前环境：{environmentLabel}<br />{page === 'cron' ? '此页面只读，不会运行或暂停任务。' : page === 'vision' ? '相机与训练帧只在 Mac 本机采集。' : '修改会写入该环境的教程数据'}</div></aside>{page === 'cron' ? <CronDashboard api={api} onUnauthorized={() => reportFailure(new AdminApiError(401, '后台会话已失效'))} /> : page === 'vision' ? <VisionDashboard api={api} onUnauthorized={() => reportFailure(new AdminApiError(401, '后台会话已失效'))} /> : <main className="admin-main">
