...
describe('TsumegoProgressProvider — real, resolved user (Alice)', () => {
  ...
  it('uses cookie auth and calls getProgress when there is a user but no token', async () => {
    renderHook(() => useTsumegoProgress(), { wrapper });
    await waitFor(() => expect(mockGetProgress).toHaveBeenCalledWith(undefined));
  });
  ...
  it('markProgress uses cookie auth when there is a user but no token', () => {
    const { result } = renderHook(() => useTsumegoProgress(), { wrapper });
    act(() => {
    ...
    expect(mockSaveProgress).toHaveBeenCalledWith(
      'p9',
      { completed: true, attempts: 1, lastDuration: undefined },
      undefined,
    );
...
describe('TsumegoProgressProvider — legacy unscoped cache', () => {
  it('drops unowned legacy data for a real user instead of attributing it to them', () => {
    seedLegacy({ p1: { completed: true, attempts: 1 } });
    mockUseAuth.mockReturnValue(aliceAuth());
    ...
    expect(result.current.progress).toEqual({});
    expect(readNamespaced(ALICE).p1).toBeUndefined();
