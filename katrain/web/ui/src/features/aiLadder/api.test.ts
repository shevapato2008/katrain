...
describe('ai ladder API', () => {
  ...
  it('treats a released reservation as the success it is, not as a failed request', async () => {
    ...
    // 象棋今晚踩的正是这一格:后端成功、云端已落账,而界面的解码器认不出这个形状,
    // 于是屏上告诉用户「失败了」。围棋这边形状不同、结局一样:释放**没有 receipt**
    // (它不写账本 —— 「那台盒子再没回来」不是裁决),于是旧的守卫把 200 当成畸形响应,
    // `createApiError` 拿着 `response.ok === true` 造出一个 `Request failed 200`,
    // 用户看到「结束对局失败，请重试」—— 而那一刻预约已经删掉了、账号已经放开了。
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({
  ...
  it.each([
    ...
    // `counted` 必须是**假**:释放的全部意义就是什么都不计。一个说自己计了分的释放
