import { Box, Typography, Divider, IconButton, Stack, Slider, Tooltip, Tabs, Tab } from '@mui/material';
...
export default function ResearchAnalysisPanel({
    ...
    return (
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
        ...
        {[0, 0.5, 1].map((ratio, i) => {
          ...
          return (
            <g key={i}>
              ...
              />
              <text x={leftPad - 6} y={y} textAnchor="end" dominantBaseline="middle" fill={alpha(theme.palette.primary.main, 0.8)} fontSize="8">
                {`${(ratio * 100).toFixed(0)}%`}
              </text>
            </g>
          );
        ...
        <line
          ...
          stroke={alpha(theme.palette.primary.main, 0.3)} strokeWidth="1.5"
        />
        {/* Fill areas */}
        {fillPathBlack && (
        ...
        {/* Winrate line */}
        {pathD && (
          <path d={pathD} fill="none" stroke={theme.palette.primary.main} strokeWidth="1.5" />
        )}
  ...
  }, [history, currentMove, totalMoves, theme]);
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ flexGrow: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
          ...
          <Box sx={{
            ...
            <Box sx={{ width: `${winratePercent}%`, background: 'linear-gradient(90deg, #3a3a3a, #141414)' }} />
            <Box sx={{ width: `${100 - winratePercent}%`, background: 'linear-gradient(90deg, #efe8db, #c9c0ad)' }} />
          </Box>
        </Box>
                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      bgcolor: isActual ? alpha(theme.palette.warning.main, 0.15) : 'transparent',
                      border: isActual ? '1px solid' : '1px solid transparent',
                      borderColor: isActual ? 'warning.main' : 'transparent',
                  ...
                  >
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      <Box sx={{
                        ...
                        bgcolor: nextPlayer === 'B' ? '#000' : '#f5f5f5',
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Box sx={{
                        ...
                        bgcolor: isActual ? 'warning.main' : 'primary.dark',
                        borderRadius: 1,
                      }}>
                        <Typography variant="body2" fontWeight="bold"
                          color={isActual ? 'warning.contrastText' : 'primary.main'}
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
                      <Box sx={{
                        ...
                        borderColor: nextPlayer === 'B' ? '#000' : '#666',
                      }}>
                        <Typography variant="body2" fontWeight="bold" sx={{ lineHeight: 1, fontSize: '0.8rem' }}>
                          {(wr * 100).toFixed(1)}
                        </Typography>
                      ...
                      <Box sx={{
                        minWidth: 40, height: 24, px: 0.5,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        bgcolor: nextPlayer === 'W' ? '#000' : '#f5f5f5',
                        color: nextPlayer === 'W' ? '#fff' : '#000',
                        borderRadius: 1, border: '1px solid',
                        borderColor: nextPlayer === 'W' ? '#000' : '#666',
                      }}>
                        <Typography variant="body2" fontWeight="bold" sx={{ lineHeight: 1, fontSize: '0.8rem' }}>
                          {(oppWr * 100).toFixed(1)}
                        </Typography>
            ...
            {trendTab === 0 && (
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5, px: 0.5 }}>
                  ...
                  <Typography variant="body2" sx={{ color: 'warning.main', fontWeight: 600, fontSize: '0.875rem' }}>
                  ...
                  goodMoves.map((m) => (
                    <Box
                      ...
                      sx={{
                        ...
                        cursor: 'pointer',
                        bgcolor: m.moveIndex === currentMove ? alpha(theme.palette.primary.main, 0.15) : 'transparent',
                        '&:hover': { bgcolor: 'action.hover' },
                      }}
                    >
                      <Box sx={{
                        width: 16, height: 16, borderRadius: '50%', flexShrink: 0,
                        bgcolor: m.player === 'B' ? '#000' : '#f5f5f5',
                  ...
                  badMoves.map((m) => (
                    <Box
                      ...
                      sx={{
                        ...
                        px: 1.5, py: 0.75, mb: 0.5, borderRadius: 1,
                        cursor: 'pointer',
                        bgcolor: m.moveIndex === currentMove ? alpha(theme.palette.error.main, 0.15) : 'transparent',
                        '&:hover': { bgcolor: 'action.hover' },
                      }}
                    >
                      <Box sx={{
                      ...
                      <Typography variant="body2" sx={{
                        color: m.delta <= MISTAKE_THRESHOLD ? 'error.main' : 'warning.main',
                        fontWeight: 600,
                      }}>
                        {t('research:points', '{delta} 目').replace('{delta}', m.delta.toFixed(1))}
                      </Typography>
      ...
      <Divider />
      <Box sx={{ pt: 1, pb: 1.5, px: 2, bgcolor: 'var(--raise2)' }}>
        <Box sx={{ px: 1, mb: 0.5 }}>
          <Slider
            value={currentMove}
            min={0}
