...
//
// 实测(**2026-08-12,换字体之后重打**,1024×600):
//
//   01-reserved 216 / 02-active-current 216 / 03-interrupted 188 / 04-active-other 216
//   05-pending-retrying 160 / 06-pending-refused 160 / runaway-rank-name 160
//   11-devices-degraded 216
//   **10-error-state(真实文案,错误条在场)169**
//   overflow-worst-case(压力:12 倍长译文 + 16 倍错误条)**31**
//
// 另一个更紧的数:**`alertSlackPx` 在错误态和压力态都是 0** —— 错误条底边**正好压在**
// 可滚区的裁切线上。它是这块屏当前最紧的一处,比上表任何一个 emptyGap 都小。
// `actionsSlackPx` 恒为 16,那是右栏的 padding,**不是可以拿来用的余量**。
//
// ⚠️ **上一版这张表是错的,而且错了两轮没人发现。** 它写的是
// 「01-reserved 271 / 02 215 / 03 271 / 04 271 / 05 177 / 06 233 / runaway 130 /
// error 119 / worst −60(实测 2026-08-11)」—— 那是**左栏画真盘那次重设计之前**量的,
// 重设计(`20f36470`)动了整块右栏而我没回来重量,今天换字体又动了一次文字度量。
// 判据:**这张表是一句「我量过」的断言,凡是动了排版或几何,它就和图一样必须重打。**
// 十条全绿并不携带「表还是对的」—— 表是注释,没有任何断言读它。
//
// ⚠️ **`narrativeSlackPx` 现在恒为 −121,不要把它读成余量。** 重设计把事实格搬进了
// 不参与滚动的 `__fixed` 区,于是 `syncFactBottom` 结构性地落在 `clipBottom` 下方 121px,
// 与文案长短无关。**一个永远为负、又永远不触发任何断言的指标,和一个永远为绿的断言是同一种东西。**
// 它留着只作诊断打印;真正在守的是 `alertSlackPx >= 0` 和动作区整块在视口内那两条。
//
// ⚠️ **旧的变异标定(+8 绿 / +16 红 / +40 绿 / +70 红)是在旧排版下量的,现在一律作废。**
// 本轮**没有重新标定** —— 这块面板马上要按规范 §11 布局 A 重排到 460 宽(右栏 680→460),
// 重排之后所有余量都会再变一次,现在标一遍等于标了个马上要扔的数。
// **宁可写「没量」,也不把一个没量过的阈值写成量过的。** 下次动这块面板的几何时一并补。
//
...
//
// 变异量**必须大于闸的余量**,而「闸的余量」和给人看的那张余量表**不是一个数**:
// 表报的是各格自己的空段,闸卡的是**最紧的那一条必需信息离裁切线还有多远**。
// 拿前者去定变异量级,就会在两者之间造出一个「全绿」并把它读成闸有牙。
