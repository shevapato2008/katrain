 ...
 *
 *  ① ~~「落子」不是设置项~~ —— **这条撤销了(2026-08-23)**。原话是「全仓没有任何地方
 *    能让用户切它」,而**做题屏早就有这颗开关**(`TsumegoProblemPage` 的 `role="switch"`)。
 *    设备能不能(`isVisionEnabled`,后端给)和这一局想不想是**两段**,第一版把后一段抹掉了。
 *    现在按稿子画成两段可选:偏好存 `utils/playInput.ts`,`PlayInputGuard` 和 `GamePage` 都认它。
 *    选中态取的是**这一局实际会落在哪**(两段之和)。这一帧是**没标定过摄像头**那一态
 *    (取图机器上就是),所以「实体盘」灰着、「屏幕」选中,底下那行说得出为什么。
 *
 ...
 *
 *  ③ **AI 策略五项都有说明。** 另四条已按 `core/ai.py` 的对应策略核过；
 *    `.kiosk-opthint` 定高,切换说明不会让下面的组跳。
 *
...
test('四图:自由对弈开局设置 ←→ sample-go/shots/02-setup-free.png', async ({ page }) => {
  ...
  const r = await captureFourUp({
    ...
    implementationCaption:
      ...
      + '**「落子」两段可选**:设备能力和本局偏好共同决定实际去向;这台机器没标定过摄像头,'
      + '所以「实体盘」灰着、「屏幕」选中 · '
      + '**「我执」两项不是三项**:第三项「随机」是搬象棋骨架带来的,围棋 kiosk 和 galaxy 两处都只给黑白 · '
      + '**AI 策略五项都有说明**:按 `core/ai.py` 的真实策略描述 · '
