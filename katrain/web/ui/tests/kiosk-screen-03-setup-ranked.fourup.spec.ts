...
test('四图:升降级开局设置 ←→ sample-go/shots/03-setup-ranked.png', async ({ page }) => {
  ...
  const r = await captureFourUp({
    ...
    implementationCaption:
      ...
      + '**「对手」那一块 2026-08-26 换成外壳写法**:六种诚实状态(加载 / 出错重试 / 定级赛进度 / '
      + '已定档 / 档位不可挑战 / 成绩在途)全搬过来了,稿子只画了「已定档」那一种,这一帧取的就是它 · '
      + '没有在共享件上原地改样式(它同时是 galaxy 那屏的消费者),另起了一份外壳视图,'
      + '词和判别位仍取自 `features/aiLadder`,一条逐状态的 parity 闸钉住两边说的是同一套话 · '
