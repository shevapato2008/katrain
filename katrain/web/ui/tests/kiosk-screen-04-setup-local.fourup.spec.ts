 ...
 *
 *  ① **「落子」是真开关**,按稿子画成两段可选(2026-08-23)。第一版画成读数,理由写着
 *    「全仓没有任何地方能让用户切」——**那句话是错的**,做题屏早就有这颗开关。
 *    设备能不能 + 这一局想不想是两段,偏好存 `utils/playInput.ts`;对局那四条路由外面
 *    的 `PlayInputGuard` 和 `GamePage` 都认它 —— 选了屏幕就不再被标定守卫挡住,
 *    对局屏上实体盘那一套(识别绑定、AI 落子横幅、重标定、硬件故障条)整个不出现。
...
test('四图:本地对局开局设置 ←→ sample-go/shots/04-setup-local.png', async ({ page }) => {
  ...
  const r = await captureFourUp({
    ...
    implementationCaption:
      ...
      + '**「落子」按稿子画成了真开关**:第一版画成读数、理由「全仓没人能切」是错的(做题屏早有这颗开关);'
      + '现在偏好落进 `utils/playInput.ts`,`PlayInputGuard` 和 `GamePage` 都认它 —— 选了屏幕就不再被'
      + '标定守卫挡住,对局屏上实体盘那一套整个不出现 · '
