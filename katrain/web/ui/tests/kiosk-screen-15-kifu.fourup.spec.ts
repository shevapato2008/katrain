 ...
 *  ② 稿子的「职业直播」那一组(在首屏之下)和问候副标里的「职业直播」四个字 ——
 *    Fan 2026-09-22 裁定 **kiosk 端删掉整个直播模块,只在 galaxy 保留**。副标因此改成
 *    「名局，以及把谱摆到实体盘上」,首帧里那一行字的差异是预期。
 */
...
test('四图:棋谱 ←→ sample-go/shots/15-kifu.png', async ({ page }) => {
  ...
  const r = await captureFourUp({
    ...
    implementationCaption:
      '实现:/kiosk/kifu @1024×600 · 时钟冻 16:40 · 最近摆过三条是 fixture · '
      + '**没搬**稿子第五块「棋谱详情 · 界面未接」:那是说给读稿人听的,而且详情屏本轮已接上(见屏 16 对照台) · '
      + '**没有直播那一组**,副标去掉「职业直播」:Fan 2026-09-22 裁定 kiosk 端删掉直播,只在 galaxy 保留 · '
      + '组标题右端写真数据(共 1,234 局),不是稿子那句解释 · '
