 ...
 * 预期差异,每条都是裁定:
 *
 *  ① **四步,稿子已跟上。** 稿子原来画五步,第 2 步「采集熄灯参考帧」对应的 `dark_reference`
 *    **全仓没有任何地方写入**(只在两处「哪些 phase 算进行中」的常量集合里当摆设)。
 *    那件事确实在做,但是**每个锚点各一次、13+ 次**(`_locate_anchor` 每个点都先熄灯拍一张、
 *    亮灯再拍一张做差分),不是一个有头有尾的阶段。⇒ 实现删掉那一行、机制写进旁注;
 *    2026-09-24 稿子照实现改成四步(smartbox b813fd7e4),两边都是「第 3 / 4 步」。
 *    剩下的差异:机制那句只在实现的旁注里 —— 稿子的说明已两行,它的规范 §5 不许第三行。
 *
 ...
 *
 *  ⑥ **右栏中段可滚。** 稿子原来五步时按共享 token 算是 462 / 460 —— 差 2px 装不下;
 *    改成四步后稿子那一帧装得下了,可失败时要多一张诊断卡、旁注也比稿子多两行,照样顶破。
...
test('四图:棋盘标定 ←→ sample-go/shots/26-calib.png', async ({ page }) => {
  ...
  const r = await captureFourUp({
    ...
    implementationCaption:
      ...
      + '**四步**:稿子已照实现删掉「采集熄灯参考帧」(每个锚点各拍一次熄灯帧，不是一个阶段)，'
      + '机制那句只在实现的旁注里(稿子说明已两行，规范 §5 不许第三行) · '
      ...
      + '照画会在还没问过的时候说「未连接」) · '
      + '**右栏中段可滚**:失败时要多一张诊断卡、旁注也比稿子多两行，放不下 · '
      + '**LED 那格写「串口已连接」**（只说串口通了，视觉赛道 V4），稿子那格是「已连接」(左栏的说法)',
  });
