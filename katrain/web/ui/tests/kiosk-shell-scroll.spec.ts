 ...
 */
const boot = async (page: Page, path: string, css?: string, readySelector = '.kiosk-scrollzone') => {
...
/* ══ 屏 13 双栏题目列表 —— 20 格、两个换题入口、底部主按钮都不滚动 ══ */
test('题目列表:满编 20 格 + 换一批入口 + 做题按钮，一屏装得下', async ({ page }) => {
  await page.route('**/api/v1/tsumego/levels/*/categories/*', (route) => route.fulfill({
    json: Array.from({ length: 45 }, (_, i) => ({ id: `q${i}` })),
  }));
  await boot(page, '/kiosk/tsumego/15k/capturing/1', undefined, '.tsumego-browser__rail');
  await page.waitForSelector('.qgrid button:nth-child(20)');

  const m = await page.evaluate(() => {
    const rail = document.querySelector('.tsumego-browser__rail') as HTMLElement;
    const cells = Array.from(document.querySelectorAll('.qgrid button')) as HTMLElement[];
    const exits = document.querySelectorAll('.tsumego-browser__alternatives button');
    const action = document.querySelector('.tsumego-browser__enter') as HTMLElement;
    return {
      cells: cells.length,
  ...
  expect(m.cells).toBe(20);
  expect(m.exits).toBe(2);
  expect(m.overflow).toBeLessThanOrEqual(0);
  expect(m.actionBottom).toBeLessThanOrEqual(m.railBottom);
});

/* ══ 屏 13 —— 右栏 460 内按 5 列 × 4 行放满 20 题 ══ */
test('题目列表:.qgrid 五列铺满右栏 460，页面不横向溢出', async ({ page }) => {
  await page.route('**/api/v1/tsumego/levels/*/categories/*', (route) => route.fulfill({
    json: Array.from({ length: 45 }, (_, i) => ({ id: `q${i}` })),
  }));
  await boot(page, '/kiosk/tsumego/15k/capturing/1', undefined, '.tsumego-browser__rail');
  ...
  const g = await page.evaluate(() => {
    ...
    return {
      ...
      right: Math.round(cells[4].right),
  ...
  expect(g.gridW, '题号格没铺满右栏 460').toBe(460);
  expect(g.gridW).toBe(g.clientW);
  expect(g.perRow, '一行不是 5 格').toBe(5);
  expect(g.rowTops, '20 格没排成四行').toBe(4);
  expect(g.cellH, '题目触控区不够高').toBeGreaterThanOrEqual(44);
  expect(g.left, '第一格没贴左缘').toBe(g.gridLeft);
  expect(g.right, '第五格没贴右缘').toBe(g.gridRight);
