...
- [x] 先写失败测试：缺/弱凭据拒启；公共 access/refresh 不能进后台，后台令牌不能进公开 API；缺 `aud/type/env/exp` 拒绝；后台用户名与 Galaxy 用户隔离；失败登录不泄露具体原因。
- [x] 实现单管理员 env password hash 校验、后台独立 JWT 密钥、登录/登出/`me`、CSP/无跨站 CORS、同源静态资源。审计记登录和写操作，`actor_realm=admin`，无虚构 `users.id`。
- [x] 先写失败测试，再迁移四个教程写操作；请求先鉴权，保存遵守并发版本，服务端确定作者/审核者；公开只读资产可从后台工作台读取。必要共享业务函数只在服务层复用，不让后台 token 被公开 API 接收。
- [x] 错误路径聚焦 TTS 失败/重试、审计与业务提交一致性、本地共享媒体与训练样本路径；未搭通用任务平台。外部 Edge TTS 与真实对象存储仍待目标环境授权后的验收。
...
- [x] 本机补充聚焦验收：未保存讲解草稿在图、章、书切换后丢弃，写接口返回 401 后退回登录并清除本地令牌；`AdminApp.test.tsx` 12 项、管理前端共 53 项通过，`tsc -b` 和该文件 ESLint 通过。目标环境媒体更新与真浏览器书/章切换仍归上项待办。
  - [x] 真实 API 和 Fixture 删除已在 `252bffaa`；本机临时 SQLite＋离线合成媒体的浏览器验证覆盖已有音视频读取/播放、图切换后媒体空态、生成语音后新音频出现。外部 TTS 不在这次本地验证内；书/章切换与目标环境媒体仍待验收。
...
切片 0 安全代码与教程后台本地实现已先后合入 `develop`，但测试/生产服务仍未部署。B 夜间／C 白天＋楷体外观已获 Fan 确认，本机临时库真实 HTTP 浏览器流程、离线合成媒体播放已跑通；书/章切换的草稿隔离已有聚焦测试。**外部 Edge TTS、真实媒体存储、真浏览器书/章切换及 Fan 端到端验收仍待完成**。旧全表教程同步已在三份技能说明中标为停用，替代导入流程尚未制定。没有具体授权不能写任一真实库、连接远程、push 或部署。
