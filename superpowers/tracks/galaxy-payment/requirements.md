...
| U4 | ~~经营许可证：B25 还是 B21；"卖分析额度"是否构成经营性~~ **已定，见下方 §2.4** | 无证经营的罚则是《互联网信息服务管理办法》**第十九条**：责令限期改正、没收违法所得、3–5 倍罚款；无违法所得或不足 5 万处 **10 万–100 万**；**情节严重责令关闭网站**。已另出 prompt 交由专门调研。 |
...
### 2.4 U4 已定（2026-09-07，与 Fan 当面过完）

**结论：要办 B25（第二类增值电信业务 · 信息服务业务，仅限互联网信息服务）。卖积分不改变定性。**

- 判据在《互联网信息服务管理办法》**第三条**：经营性 = "通过互联网向上网用户**有偿**提供信息……服务"。
  用户充钱换积分、再用积分换分析，"有偿"这一步已经成立；把人民币改名叫积分不改变它。
  "会员充值"在实务清单里是最典型的要办证场景，不是灰色地带。
- **比法条更早拦住的是收款通道**：支付宝/微信支付的虚拟商品与在线服务类目在商户入驻时就查资质。
  没有证连把钱收进来那一步都走不完。
- 积分还会**多招一层**监管而不是少一层：预付余额要按预付式消费处理（退款、有效期、注销善后）。

**Fan 问的"能不能绕过 3 个人社保"——不该绕，而且它比想象中便宜。**
- 3 人社保**不在法条里**。《电信业务经营许可管理办法》（工信部令第 42 号）**第六条**只有六项条件，
  没有人数；"3 名员工近 1 个月社保"是各省通信管理局用来证明第（二）项"与开展经营活动相适应的
  ……专业人员"的**举证口径**，各省不一（多数 1 个月，部分 3 个月）。
- **北京市通信管理局现行办事指南（版本 20260116）公示的受理条件逐字就是第六条那六项，没写人数**；
  北京相对宽松。注册资本 100 万是**认缴**；办证**不收费**；受理 5 个工作日 + 决定 40 个工作日。
- 挂靠 / 借证 / 买证是**明确违法**：第 42 号令**第十九条**（不得伪造、涂改、冒用、以任何方式转让）、
  **第四十五条**（以欺骗手段取得许可 ⇒ 撤销 + 罚款 + 三年内不得再申请）。
- 成本对照：办证 ≈ 3 人社保几千到 1.8 万 + 两个月；不办的期望成本 = 没收违法所得 + 3–5 倍罚款，
  下限 10 万，情节严重责令关闭网站（292 号令第十九条）。

**不触发许可的替代路径（结构上不同，不是换说法）：**
1. **把算力捆进硬件售价** —— 卖自有实体商品业内共识只需备案；网站对用户免费。
   代码现成：`billing.generate_redeem_codes()` + `/api/v1/billing/redeem` 已在跑，随盒发兑换码即可。
   **边界：网站上一旦出现独立的续费/充值入口就又踩回去了。**
2. **线下 B2B 合同**（道场/学校/俱乐部，对公转账）——不是"向上网用户有偿提供"。
3. 第三方平台小程序：广东(2022)/浙江(2021)/辽宁/安徽 有公告说"无独立运营平台、仅依托微信支付宝
   终端的小程序"无需单独申请，**但北京有没有同类公告未查到**，这条要用必须先问北京。

**拿到证之后**：第 42 号令**第十六条**要求在网站主页显著位置标明许可证编号（页脚已有位置）；
未标明按 292 号令第二十二条罚 5000–5 万。每年 3 月底前年报。

**待办的一个电话**：北京市通信管理局 (010) 51938038 —— 一通电话能同时定死"要不要办"和
"人员社保几人几个月"两件事。
