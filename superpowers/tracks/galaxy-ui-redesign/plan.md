...
## S7 · 右栏加宽（2026-08-30，已完成）

**起因。** Fan 在 2000×1050 的窗口上看对局页，说「右边栏有点过于窄了，在保障棋盘可以占满
中间位置的同时，尽可能给右边栏留一些空间」，并给了星阵围棋（19x19.com）的右栏作参照
（截图存在 `visual/rail-width/_reference/`）。量过两张截图的标注框：星阵右栏 669px、
本站 434px（含标注边框），栏宽占视口比 33% vs 19%。

**问题不是审美，是死白边。** 棋盘是正方形，宽屏上它由**可用高度**封顶，棋盘列剩下的横向
空间既不给棋盘也不给右栏。真浏览器实测（`/galaxy/live`）：2000×1050 上棋盘 986，
棋盘列却有 1380——**382px 谁都用不上**；2560×1440 上 552px。

**做法。** 右栏由三档改四档，spec §2.3：
...
每一档的取值判据是**加宽之后棋盘边长不变**。900–1199 不动，是因为那一档棋盘本来就是
宽度受限的（1024×768 上棋盘列 704 已窄于可用高度 716），任何加宽都要棋盘出。
十一档实测里八档棋盘一像素没动，三处例外（1200×800 −20、1280×800 −20、1536×960 −32）
都是白边为零的档，让步已压到 4% 以内。完整的量表在 spec §3.3。

**顶档止于 520 —— 卡住它的是图表，不是棋盘。** 独立审核指出并且核实无误：2560×1440 上棋盘
另有一个上限（`components/Board.tsx:118` 的 `Math.min(1200, …)`），所以那一档右栏再加宽
**一个像素都不花棋盘的**，「棋盘优先」在那一档根本不构成约束。真正卡住的是三个矢量图表：
viewBox 写死 420 宽、`xMidYMid meet`，栏宽一超过 420 图就不再变大，多出来的宽度全变成
图表框内部的死区。520 时还能忍，620 就明显了。**要先把图表改成按容器实测宽度驱动
（spec §2.5「已知未解」），再谈抬高顶档。**
**2560 下仍有约 410px 死白边，是已知未解项**，不装作解决了。

**栏内跟着改了三处**（新增 spec §2.5 为这两类立规则）。这三处**都是独立视觉审核逼出来的**
——第一版实现只做了「分列」和「图表铺满」，两条都被审核当场证伪，见下面「审核推翻的」。

1. **显示开关**（`RightSidebarPanel.tsx` 一组三个 / `LiveMatchDisplayControls.tsx` 与
   `TsumegoProblemControls.tsx` 各一个）：装得下第二列时 `auto-fit` 分列，**同时关掉
   `space-between`**。规则出自新建的 `src/components/railStyles.ts` 一处，四个棋盘页共用。
   实测（真浏览器，滑块到「自己的标签」/「下一列的标签」的距离）：
   520 档 12 / 163，899 堆叠态 12 / 358；320/360/430 仍是单列、两端对齐照旧（205/245/326）。
2. **收窄带统一**：`PlayerCard.tsx`（899）、`PlaybackBar.tsx`（340）、`KifuLibraryPage.tsx`（340）
   三处各写各的，全部换成 `railStyles.ts` 的 `RAIL_TIGHT`（460）。
   两处 340 是照着旧的「1200–1535 档右栏 340」定的，**那一档一改成 360 就会悄悄失效**
   ——审核实测证实了，不改就是一次没人看得见的行为变更。
3. **`PlayerCard` 的容器带拆成两条**：语义（呼吸点、名字不截断、非行棋方压暗）留在 899，
   字号与内边距那一组移到 `RAIL_TIGHT`（460）。原来合成一条，前提是它自己注释里写的
   「右栏最宽 380、两张卡各得约 140」；加宽到 520 之后每张卡有 244，却仍按 140 的字号画。
   **代价说清楚**：堆叠态（<900，右栏是满宽段）里 461–899 这一段的卡片也跟着退回原尺寸。
   461 时每张卡约 222px，宽于当初出事的 154，实测无溢出；但这确实动了一次已批版式。

**审核推翻的（记在这里，因为这是这次最值钱的部分）：**

- **图表「铺满」已撤回。** 原做法是 `aspect-ratio` + `height:auto` 让高度跟着宽度长。
  它在 <900 的堆叠态把 SVG 里的坐标轴文字**一起**放大到 27px、页面高出 188px。
  正确解法与它的第二个坑（`ResearchAnalysisPanel` 里还有一份写死的 `svgWidth = 420`
  管点击定位，viewBox 一变就要同步）记在 spec §2.5 的「已知未解」。
- **分列的第一版是反向优化。** 只加 `auto-fit`、留着 `space-between`，520 档下每个滑块
  离下一列的标签 16px、离自己的标签 159px，邻近性整个反过来。顺带一个坑：
  `'& > *'` 的特异性是 (0,1,0)（通配符不计），与子元素自带 `sx` 生成的类打平，靠源序决定，
  实测输了 —— 要写成 `'&& > *'`。

**没做的（记账，不是遗漏）：**

- 全站扫出 135 处宽度敏感写法（39 处 space-between、18 处四宫格、20 处为 380 调过的写死像素）。
  本轮只修在 520 档下**真的坏掉**的那几处。
- 2560×1440 上仍有约 410px 死白边。棋盘还被 `Board.tsx:118` 的 `Math.min(1200, …)` 封在 1200，
  所以那一档再加一级右栏是**不花棋盘一个像素**的；没做，是因为 620 只在栏内修好**之前**
  试过一次，修好之后没有再重测，来不及在这一轮闭环。
- 栏内下方的大片空白（2560 下约 530px）没动。
- `TsumegoProblemPage` / `GameRoomPage` / `TutorialFigurePage` 三页本地起不出右栏（缺数据），
  它们的栏宽是**论证**出来的（同一个 `BoardPageShell`、宽度字面量全仓只有那四行），不是量出来的。
- 1024×768 下右栏内容溢出 108px 需要滚动 —— **旧宽度下逐像素相同**，既有问题，本轮未动。

**闸。** 栏宽这个数出现在五处，全部同步改了：
`BoardPageShell.tsx`（真源）、`BoardPageShell.test.tsx`（单测钉四档 + 钉「没有第五档」）、
`tests/galaxy-live-template.spec.ts`（真浏览器逐档量，含各断点前后 1px 和 2560 顶档）、
`loadbearing_board_page.js`（六个棋盘页共用的承重探针）、spec §2.3/§3.3/§2.5。

**取图与实测脚本。**
- `capture_rail_widths.mjs` 在同一次构建下出 before/after 两套（before 用 CSS 覆盖把栏宽压回
  旧三档，所以两张图之间除栏宽外无其他变量），`compose_rail_width.py` 合成并排图与差异图，
  产物在 `visual/rail-width/` 下按视口分目录。参考图（Fan 的原截图与星阵围棋）在
  `visual/rail-width/_reference/`。
- `audit_rail_width.mjs`（`npm run audit:rail`）逐页逐档量 `board-right-rail` 的实际宽度，
  7 个棋盘页 × 9 档共 63 次，全部与 spec §2.3 相符。它补的是
  `tests/galaxy-live-template.spec.ts` 在本地跑不了的那一块——那条 spec 要真实直播数据，
  本地 20/21 条卡在 `canvas` 渲染不出来（**基线构建下同样如此，与本次改动无关**）。
