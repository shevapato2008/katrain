...
### C38. 闸改 0.550 后的级位段重排：41 档 → 29 档可坐（2026-08-20）

**五批共 4,435 局**（2,298 + 1,458 + 551 + 19 + 109），除末批被主动中止外 0 失败，每批开跑前落盘 `PREREGISTRATION.md` 与
`engine_health_prelaunch.json`。引擎全程 home-ubuntu `katago-calib`（TensorRT，8010，humanv0）。

末批（段位段 confirm）跑到 19 局时按 Fan 指示停跑，那 19 局**已按常规口径并入**：
停在哪条缝由调度决定、与任何结果无关（停跑前未读过本批任何数字），所以不构成选择偏差。
它使段位段几条边的 n 从 100 变成 102。

#### C38.0 两条裁定

Fan 2026-08-20，先后两次：

> 「级位段这些可以缩减到15档，你自己在home-ubuntu上做实验，上一级对下一级胜率保持在>=60%即可。」
> 「这个胜率太高了。既然15档排不了，我们把胜率边界改成55%。」

⇒ **级位段判据 = 0.550 点估计**（段位段沿用 C37 系列的数据，本轮未重判）。
第二条的动机要记住：他嫌的是**步子太大**（链上 0.65–0.79），降闸门是为了让阶梯更平滑，
不是为了放宽标准 —— 方向与「降闸门=放水」相反。

#### C38.1 级位段档案轴的真实上限

在 20 个 `rank_Nk@1` 节点上跑最长链（只认实测边，n≥100）：
...
**0.525 比 0.550 还少** —— 因为挡路的是**倒挂**不是分辨力（`17k→16k` 0.467、`14k→13k` 0.500、
`6k→5k` 0.492）。降闸门救不了倒挂，这条可以推广到任何棋类的阶梯排布。

Bradley-Terry 拟合（MM 迭代，31 条边 3,480 局，残差 0.0081）给出的档案 Elo（20k=0）：

```
19k  -4   18k +10   17k +54   16k +45   15k +82   14k +144  13k +156
12k +240  11k +292  10k +344   9k +399   8k +472   7k +543   6k +629
 5k +627   4k +785   3k +897   2k +958   1k +1073
```

**整个级位段只有 1,073 Elo**，而真实 20 级→1 级约 1,900 ⇒ humanSL 在 1 visit 加权采样下
把级位压缩了将近一半。这才是「20 个名字排不满 20 个档」的物理原因。

⚠️ **拟合值是外推，只用来决定下一步测哪条边，绝不当过闸证据。** 两次实测打脸：
拟合说 `20k→18k` 0.567，实测 **0.490**；拟合说 `16k→15k` 0.554，实测 0.533。

#### C38.2 温度轴（T>1）在 `rank_20k` 之下接档 —— 失败

想法：`rank_20k` 已是 humanSL 最弱档案，用 `human_policy_temperature > 1` 摊平策略，
在它**下面**再接几档。段位段数据给出模型 `Elo ≈ 870·ln(1/T)`（t1→t1.15 实测 +95~+147 对
预测 121 ✓；t1→t2 实测 +510~+800 对预测 603 ✓，两点独立吻合）。

**该模型在 `rank_20k` 上被证伪。** 实测响应是三段式：

| 缝 | 胜率 | conclusive |
|---|---|---|
| t1.0 → t1.12 | **0.490** | 0.910 |
| t1.12 → t1.26 | 0.770 | 0.924 |
| t1.26 → t1.41 | 0.710 | 0.741 |
| t1.41 → t1.58 | 0.620 | 0.692 |
| t1.58 → t1.78 | 0.670 | 0.627 |
| t1.78 → t2.0 | **0.510** | 0.629 |
| 锚 t1.0 → t2.0 | 0.940 | 0.694 |

**t≤1.12 完全没效果，t≥1.78 饱和，整带只有 490 Elo**（模型预测 603）。

真正的否决项是 **conclusive 率**。要分两种读法，这条在开跑后、读数前就补进了预声明：

- **两边都热**的自对弈 conclusive 低是**伪影**，不作否决 —— 现役 `rank_19k@1 vs rank_20k@1`
  实测 move_cap 已经 0.727、conclusive 0.899，撞 400 手上限是弱档固有属性。
- **产品风险**只看**热档 vs 常温档**（那才是对人类的条件）。实测：
  t1.26 → **0.868** ✅ / t1.41 → 0.784 ❌ / t1.58 → 0.804 ❌ / t1.78 → 0.807 ❌。

⇒ **t≥1.41 三成棋下不完，是产品缺陷（对手不会收官），出局。** 温度轴最多只能加 1 档
（`rank_20k@1t1.26`，常温档打它 0.820 —— 会成为全表最大的一跳），因此**没有采用**。

#### C38.3 贪心搜索策略与穷尽性审计

Fan 提出的策略：铆钉 20级，每次取**最近的、胜率 ≥0.550 的**下一档。在「强度全序、胜率随
Elo 差单调」的前提下这是**最优贪心**（取最近合格者给后面留最多空间）。

DP 最长路在**已测边**上等价于它。差别只在：DP 把没测过的对当成不存在。因此逐条边审计
（`greedy_audit.py`）：13 条链边里 **8 条连接相邻档案**（中间无节点可插，不是没测），
4 条的可插节点全部被实测挡死，**只剩 `17k→15k` 一处未排除** —— 补测得 **0.530**，不过闸。

⇒ **搜索空间已穷尽。级位段 14 档是节点数的上限，不是策略的上限。**
要再多档必须**造新节点**（例如把更强档案调钝去插进宽缝），本轮 Fan 裁定不做。

#### C38.4 最终阶梯：29 档可坐 / 12 档封档

下表**可从仓内文件原样复算**（覆盖 178 个配置对 / 17,918 局，与出表用的输入集合完全相同）：

```
cd superpowers/tracks/golaxy-ai-ladder-parity/calibration
python replay_seams_summary.py \
    --summary results/artifacts/ladder_all_seams_20260820.jsonl.gz \
    --by-config --pairs --gate-point 0.550
```

存活 29 档，最小步 **0.570**，无一条低于 0.550：

| rung | 档位 | 网络 | 配方 | 打下一档（胜率） | n |
|---|---|---|---|---|---|
| 1 | 20级 | humanv0 | `rank_20k@1` | （最低档） | — |
| 4 | 17级 | humanv0 | `rank_17k@1` | **0.570** vs 20级 | 100 |
| 7 | 14级 | humanv0 | `rank_14k@1` | **0.630** vs 17级 | 100 |
| 9 | 12级 | humanv0 | `rank_12k@1` | **0.615** vs 14级 | 200 |
| 10 | 11级 | humanv0 | `rank_11k@1` | **0.573** vs 12级 | 220 |
| 11 | 10级 | humanv0 | `rank_10k@1` | **0.595** vs 11级 | 220 |
| 12 | 9级 | humanv0 | `rank_9k@1` | **0.600** vs 10级 | 220 |
| 13 | 8级 | humanv0 | `rank_8k@1` | **0.595** vs 9级 | 220 |
| 14 | 7级 | humanv0 | `rank_7k@1` | **0.591** vs 8级 | 220 |
| 15 | 6级 | humanv0 | `rank_6k@1` | **0.625** vs 7级 | 320 |
| 17 | 4级 | humanv0 | `rank_4k@1` | **0.720** vs 6级 | 200 |
| 18 | 3级 | humanv0 | `rank_3k@1` | **0.650** vs 4级 | 320 |
| 19 | 2级 | humanv0 | `rank_2k@1` | **0.591** vs 3级 | 220 |
| 20 | 1级 | humanv0 | `rank_1k@1` | **0.623** vs 2级 | 220 |
| 22 | 1段 | humanv0 | `rank_1d@1` | **0.676** vs 1级 | 102 |
| 24 | 2段 | humanv0 | `rank_2d@1` | **0.683** vs 1段 | 120 |
| 26 | 3段 | humanv0 | `rank_3d@1` | **0.650** vs 2段 | 120 |
| 28 | 4段 | humanv0 | `rank_4d@1` | **0.667** vs 3段 | 120 |
| 30 | 5段 | humanv0 | `rank_5d@1` | **0.600** vs 4段 | 120 |
| 32 | 6段 | humanv0 | `rank_7d@1` | **0.686** vs 5段 | 102 |
| 33 | 准7段 | humanv0 | `rank_7d@1t0.8` | **0.783** vs 6段 | 120 |
| 34 | 7段 | humanv0 | `rank_7d@1s` | **0.765** vs 准7段 | 102 |
| 35 | 准8段 | b18 | `rank_7d@2` | **0.637** vs 7段 | 102 |
| 36 | 8段 | humanv0 | `rank_9d@1s` | **0.578** vs 准8段 | 102 |
| 37 | 准9段 | b18 | `rank_9d@2` | **0.657** vs 8段 | 102 |
| 38 | 9段 | b18 | `rank_9d@4` | **0.750** vs 准9段 | 100 |
| 39 | 职业水平 | b18 | `b18@1` | **0.625** vs 9段 | 120 |
| 40 | 职业顶尖 | b18 | `b18@12` | **0.842** vs 职业水平 | 120 |
| 41 | 超越人类 | b18 | `b18@64` | **0.925** vs 职业顶尖 | 120 |

封档 12 档 —— 每一档挡它的那条实测边：

| rung | 档位 | 配方（保留） | 挡它的实测 | 值 | n |
|---|---|---|---|---|---|
| 2 | 19级 | `rank_19k@1` | 20级→19级 | 0.525 | 120 |
| 3 | 18级 | `rank_18k@1` | 20级→18级 | 0.490 | 100 |
| 5 | 16级 | `rank_16k@1` | 17级→16级 | 0.467 | 120 |
| 6 | 15级 | `rank_15k@1` | 17级→15级 | 0.530 | 100 |
| 8 | 13级 | `rank_13k@1` | 14级→13级 | 0.500 | 120 |
| 16 | 5级 | `rank_5k@1` | 6级→5级 | 0.492 | 120 |
| 21 | 准1段 | `rank_1d@1t1.15` | 准段轴反序（C37.2） | 0.346–0.515 | 120 |
| 23 | 准2段 | `rank_2d@1t1.15` | 准段轴反序（C37.2） | 0.346–0.515 | 120 |
| 25 | 准3段 | `rank_3d@1t1.15` | 准段轴反序（C37.2） | 0.346–0.515 | 120 |
| 27 | 准4段 | `rank_4d@1t1.15` | 准段轴反序（C37.2） | 0.346–0.515 | 120 |
| 29 | 准5段 | `rank_5d@1t1.15` | 准段轴反序（C37.2） | 0.346–0.515 | 120 |
| 31 | 准6段 | `rank_6d@1t1.15` | 准段轴反序（C37.2） | 0.346–0.515 | 120 |

**封档 ≠ 删除配方。** 12 档全部保留 catalog 位置与配方，只是 `PLAYABLE_RUNGS` 不含它们
（`ladder._RETIRED_RUNGS`）—— 这样不可变账本里已存的 rung 号仍指向它当初那一档，
退役前结算的对局依然可解释。

**级位段这一半没有改动任何配方**：代码里 rung n 的配方一直就是 `rank_{21-n}k`，
存活的 14 档每一档正好等于它自己的名字。**封档本身就是全部交付。**

#### C38.5 封档接线：定级中点必须同时 snap

接线时踩到一件必须成对落地的事。定级搜索走**原始** rung 号，它的中点会点到封档档位 ——
从 1..32 可达的 31 个窗口里有 **12 个**（实测变异得到，不是估计）。那些局根本坐不上人。

修法两半，**缺一不可**：

1. `expected_opponent_rung` 把中点 snap 到最近可坐档位。
2. 原先另外**三处内联复制**了同一个中点公式（`ai_ladder_ranked.py` 结算错配检查两处 + 定级游标一处）
   收敛到那唯一定义。只 snap 一半会让**合法坐上的对局在结算时判 `opponent_rung_mismatch`**，
   玩家静默地不再涨分。

**二分游标故意保持原始**（`_apply_result` 里的 `pivot`）：把游标也 snap 会让 `placement_lo`
越过 `placement_hi`（第 5 局时窗口已窄到 1–2 格，可能一个可坐档位都不含）。代价是搜索
「以为自己测了 mid，实际测的是 snap(mid)」，相邻可坐档位最大间隔 3 ⇒ 偏差 ≤2 档，
而定级本身是 5 局分辨 32 档。落点另外再 snap 一次。

闸：`test_every_reachable_placement_window_seats_a_playable_rung`
（变异验证：去掉 snap 立刻红，报 12 个坐不上的窗口）。

#### C38.6 方法论：本轮新增的可复用结论

1. **重跑同一 matchup 进新目录 = 真独立样本。** 同 `(pair, color)` 位胜负一致率
   57/104 = **0.548**，正好是 p≈0.6 下两次独立抽样的理论值 0.52（若是同一盘棋应为 1.00）。
   抽签由每批不同的 `manifest_sha256` 派生。⇒ 先前所有合并 n **不是**重复计数。
   开局套件与抽签都是确定性的，**没有种子旋钮**，所以这条必须实测而不能假设。
2. **两条候选路线要在测之前定死主选**，否则测完挑高的那条就是选择效应。本轮
   17k / 15k 两条路线按 maximin 事先定主选 17k，测完 17k(0.570/0.630) 与 15k(0.640/0.564)
   都过闸，仍按预定采用 17k。
3. **闸门变了会换一批承重边。** 0.600 下不上链的边（n=120）在 0.550 下条条承重，把它们
   补到与其余边同样的 n 是**消除样本量不齐**，不是「加 n 直到过闸」—— 区别在于是否
   **一次全跑、结果一次全收**，且事先写死「无论高低一律并入」。
4. **"退役/冻结"的语义必须钉死常量。** 8/19 把 `_TEMPERATURE_CANDIDATES[0]` 从 `t1.15`
   改成 `t0.8`，已退役的准1–准6段跟着变成**锐化**，于是每档都强过正上方的 N段 —— 六处倒挂，
   且因为封档当时未接线而是**活的**。已加 `_RETIRED_QUASI_DAN_TEMPERATURE = 1.15` 解耦。
...
| §C38.2 温度带 6 缝 + 锚缝 + 档案链复现 8 缝(2,298 局) | `calibration/results/ladder_kyu15_20260820/` |
| §C38.1 闸 0.550 承重边:3 新 + 5 补 n + 4 温度对常温(1,458 局) | `calibration/results/ladder_kyu550_20260820/` |
| §C38.3 第 14 档主选/备选路线 + 12k 承重 confirm(551 局) | `calibration/results/ladder_kyu14_20260820/` |
| §C38.3 贪心穷尽性:唯一未排除的 `17k->15k`(109 局) | `calibration/results/ladder_greedy_gap_20260820/` |
| §C38 段位段 confirm(**跑到 19 局时按 Fan 指示停跑**;已并入) | `calibration/results/ladder_confirm_20260820/` |
| §C38 **全批次逐局派生摘要**(17,918 局 / 178 配置对 / 92.8 KB,§C38.4 表由它复算) | `calibration/results/artifacts/ladder_all_seams_20260820.jsonl.gz` |
