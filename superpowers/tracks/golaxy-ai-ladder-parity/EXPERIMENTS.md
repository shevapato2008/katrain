...
| **6d/5d** | **5–15 / 11–9** | **0.25 / 0.55** | 0.112–0.469 / 0.342–0.742 | **两次矛盾** |
...
- [ ] **6d/5d 接缝方向未定**:§C35 同指纹两次跑出 0.25 与 0.55。已证实该配置非确定性(20/20 槽位手数
  不同、9/20 胜负不同),差异属采样方差。需第三轮加厚(引擎在线时可同参数 resume 续跑,
  `--max-pair-attempts 30` 必须保持不变,否则指纹失配无法续)。同区间 5d/4d、8d/7d 亦为 10–10 持平。
