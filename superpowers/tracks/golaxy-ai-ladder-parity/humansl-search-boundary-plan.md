...
- [x] Source-revision fingerprinting is implemented and approved in commit `b72451b01169560a44d2ad6f5e6e4cfbca7d5007`. Pin the initial `@20` launch to exactly that source revision; later documentation commits must not be substituted.
- [ ] Create a separate worktree with `git worktree add --detach /tmp/katrain-exp3-boundary-launch b72451b01169560a44d2ad6f5e6e4cfbca7d5007` and enter it. Run `uv run python i18n.py`, then require `test "$(git rev-parse HEAD)" = "b72451b01169560a44d2ad6f5e6e4cfbca7d5007"`, `test "$(git rev-parse --abbrev-ref HEAD)" = "HEAD"`, and `test -z "$(git status --porcelain=v1 --untracked-files=no)"`. The only allowed `--out` is the external absolute original-workspace path `/Users/fan/Repositories/katrain-golaxy-ai-ladder-parity/superpowers/tracks/golaxy-ai-ladder-parity/calibration/results/selfplay_v2_pikl_boundary`.
- [ ] Run the local probe:
...
- [ ] Launch:
  From the verified clean detached worktree at `b72451b01169560a44d2ad6f5e6e4cfbca7d5007`:
