...
判据是**非黑采样点**,**写在调用点里,没有做成通用 helper**:全仓今天只有**一处**取图会渲染
`LiveBoard` —— `galaxy-ai-ladder-game-visual.spec.ts`(kiosk 开局设置屏改布局 A 之后左栏是
**内联 SVG**;galaxy 的 AI 设置页没有盘)。一个调用点做成 helper,等于替将来那个还没出现的
调用点提前定形状。B 块给 `/kiosk/play` 取图时会是第二个,到时再看要不要抽。

> 🔴 **在那唯一的消费者身上我没能把它推红。** 那条 spec 用 `page.route` 把 5 张 PNG 从
> **本地磁盘** fulfill,几乎立刻到齐,拍不到空帧。⇒ 记成「**已接线、未演示有牙**」,
> 不是「取图侧已经修好」。量到 4/6 空的那组数在 `/kiosk/play`(资产走 HTTP)。

**范围更正**:协调方最初点的是「和裁定同一批把取图侧改掉」,后来自己收窄了 ——
`01-setup-my-black` 那块盘是内联 SVG,`waitForSelector` 对它充分,那里没有竞态。
另外协调方那封说「只剩 `kiosk-ai-ladder-layout-a-geometry.spec.ts` 还在量 `/kiosk/play`」,
**实测不成立**:那条 spec 三处 `goto` 全是 `/kiosk/play/ai/setup/ranked`;
全仓访问 `/kiosk/play` 的只有 `kiosk-font-routing.spec.ts`,而它**不取图**。
...
⇒ **挂到设备走查**,判据可执行(不是「看一眼觉得还行」):

> 上板时在 `/kiosk/play` **冷启动**(清缓存、首次进页)看那块卡,**秒表计从页面出现到盘画出来
> 的时长,跑两次**。**看得见空白**就是真 bug;**两次都看不见**就把这条销掉,
> 并写明「**RK3562 上量过,没复现**」。

**「看不见」也要写来源** —— 不写的话下一个人读到的是「这条没人查过」,而不是「查过,没复现」。
又是那条**否定不携带原因**。


### 16.3 那道闸的牙:**演示过了,而且演示的结果是「它在那儿装不上」**

协调方提的方法(在 `page.route` 里塞人为延迟,把「图还没到」造出来)照做了,**一次性探针,已删**。
结果不是「红了」也不是「造不出来」,是**第三种**:

| 做了什么 | 量到什么 |
|---|---|
| 给全部 6 个 `/assets/img/*.png` 的 route handler 各塞 **12 秒**延迟 | 日志确认**六个 handler 全部命中**;这条 spec 仍然 **5.2 秒通过** |

⇒ 原因不是「资产太快」,是**那一屏画盘的根本不是 `LiveBoard`**:
它用 `components/Board.tsx`,同样 `Promise.all` 预加载 5 张 PNG,
**但在图到齐之前就已经把底和格线画上了** —— 那屏**没有全黑那一帧**。
⇒ 「非黑像素」这条判据在那儿**永远不会红**。**已把那段等待拿掉** ——
挂一段结构上不可能生效的闸,比不挂更坏:它会被下一个人读成「这屏有保护」。

**这条覆盖了协调方原来的指令**(他点这条 spec 时手里没有这个测量)。要放回去说一声。

真正的空盘证据仍然在 `/kiosk/play`(`LiveBoard`,绘制整段被 `imagesLoaded` 挡着,6 次里 4 次全黑)。
**那段等待属于 B 块**,到那时才有被测对象。

> ⚠️ 顺带量出来一件**那条 spec 真正缺的闸**(没做,记账):`Board.tsx` 的 `img.onerror` 是
> **reject**,图挂了 `renderBoard()` 就不再被调用 ⇒ 屏上是「**有格线、没有子**」。
> **「非黑像素」抓不到它,`toBeVisible()` 更抓不到** —— 要抓得钉**子在不在**。
> 那一屏的产物属于 galaxy 那条 track,不在本轮范围。
>
