...
> ## ⚠️ 2026-09-22 Fan 改判:kiosk 端删掉整个直播模块
>
> 「在 kiosk 界面直接删掉直播模块,不要把它融入棋谱库了。在 galaxy 模块保留就好。」
>
> - **推翻**下文 §4 的 L1 方案 B(补入口并接壳),也**不是**方案 A(只删 `/kiosk/live`):
>   kiosk 上的直播列表屏、观战屏(屏 18)、棋谱屏(屏 15)里的「职业直播」那一组**全部删除**,两条路由一起去掉。
> - galaxy 的直播(`src/galaxy/**`、`src/pages/**`、`components/live/*`、`api/live.ts`)**一行不动**。
>   `LiveBoard` / `types/live` 是复盘、研究、摆谱在用的共享件,也不动。
> - 屏 15 问候副标「名局、职业直播，以及把谱摆到实体盘上」随之改为「名局，以及把谱摆到实体盘上」(11 语种同步)。
> - **L2(观战屏的钟)对 kiosk 不再成立**:屏 18 没了。`findings-clock.md` 留作记录(galaxy 若要画钟可用)。
> - **L3 / L3-评论**对 kiosk 同样不再成立。
> - 盒上直播代理 `/api/v1/board/live/*`(后端)从此没有前端消费者;本轮**不删**,记为后续清理候选。
>
> 以下正文保留为决策记录,**不再执行**。
