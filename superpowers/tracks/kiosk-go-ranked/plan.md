...
| 2026-09-25 | 图标对齐 | 原稿 `smartbox-software/superpowers/shared/kiosk-shell/sample-go/go-kiosk.tmpl.html` 与实际 `GameControlPanel.tsx` 共用 `grid-nine` 领地、`squares-four` 数子、`hand-pointing` 停一手、`flag` 认输；SVG 字节与共享素材一致。按用户截图复核后，发现预览误用 24 px，已改为原稿 `tokens.css` 的 18 px、图标文字间距 4 px、按钮间距 7 px，并重新截取 1024×600 图。设计其余调整待用户说明。 |
