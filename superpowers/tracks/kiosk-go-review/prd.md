...
- **期望**:
  ...
  - **实施后终审修订(2026-09-15,代码为准)**:
    - 原因词「连不上云端」改为「云端暂时不可用」:盒上 `_remote_only` 把云端不可达**和**云端 ≥500 都转成 503,说「连不上」是猜。
    - 「重算没发出去」改为「重算没成」:402 / 400 是请求到了、被拒绝。
    - 屏 19 报告任务告警行改为「报告任务出错了 · 原因」(裸原因「已经不在了」没有主语);列表块不写 `not_found` 原因。
    - 导入时对局已存上、只是建报告失败:对话框关闭,失败只在「生成报告」告警行说,不再说「导入失败」。
    - 原文不上屏后,kiosk 两屏在各失败处 `console.warn` 原始错误,供上板排障。
    - S1 深链:谱为空、谱解析失败也写「这一局读不到」;返回路径只收纯数字 `task` / `kifu_id`。
    - 下面验收 4、5 里的「连不上云端」「重算没发出去」按上面读。
- **验收**:
