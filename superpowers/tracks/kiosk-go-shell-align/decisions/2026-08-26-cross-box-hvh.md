...
# 六、2026-08-27 追加:Fan 驳回「LED 指令下行」,以及由此长出的一条总则

## Fan 的驳(原话)

> 「LED 指令拉下来」什么意思?总不能让云端给 LED 发光下达指令吧。如果用户使用的是棋智盒,
> 给 LED 矩阵的发光指令应该从 rk3562 发出。

**成立,而且理由比「不该这么设计」更硬:云端结构上算不出来。**

LED 点位不是棋局状态的映射,**它是「数字盘面」与「摄像头这一帧看到的真实盘面」的差集**:

```
katrain/web/core/physical_play_orchestrator.py::_tick_once   @fd84f286
  observed = self._vision.get_detected_board()                 # 摄像头这一帧
  if observed is None: self._apply_points([]); return          # 看不见 ⇒ 灭灯
  expected = game_state_stones_to_board(state["stones"], board_size)
  plan     = self._planner.tick(expected, np.asarray(observed))
  self._apply_points(plan.points)
```

⇒ 云端没有摄像头、没有几何标定、没有那张 361 颗 WS2812 的 LUT,连**「这一帧看得见看不见」**
都不知道 —— 而那本身就是一个 LED 决策输入。**不是不该算,是没有输入、算不了。** 对四家都成立:
任何一家的 LED 都是「数字盘面 vs 物理盘面」的差,而物理那一半只在盒子里。

**这一改让那份裁决更强不是更弱**:它选定形状的卖点本来就是「云端一个字不改」,
而「下行 LED 指令」恰恰要改云端(得知道实体盘存在、得有 LED 协议、得进 wire 契约)。
国象已改三处(`:15`/`:89`/`:114`)并新增 §3.3 收录 Fan 原话。
...
| 方向 | 过网 | 不过网 |
|---|---|---|
| 盒 → 云 | 「我这边落了一手」·「我这块盘现在通/不通」 | 摄像头帧、标定、LUT、LED 点位 |
| 云 → 盒 | 「权威盘面是这样」 | **任何硬件指令** |

**两边都只报自己知道的事实,谁也不指挥对方的硬件。**

## 🔴 我在这一封里说过头的半句(国象驳回,我认)

我写过「D11 那是盒上自己的状态,**本来就不该问云端**」。**前半对,后半错。**

**房间屏在云端 origin 上,它读不到盒上任何东西 —— 那正是本次复审的起点**(公网页面够不到
loopback,就是待测的 (A))。⇒ **盘况必须上行**(盒 → 云 → 页面那条已有的 WS),
只是它是**事实**不是指令。

**判据(替换掉我那句):**

> **一样东西该不该过网,看的是「接收方缺不缺这一半输入」,不是「它属于谁」。**

| | 判据 | 结论 |
|---|---|---|
| LED 点位 | 云端**缺 `observed` 这一半输入** | 算不了 ⇒ 永远在盒上算 |
| 盘况 | **盒子独知、而页面需要** | **必须过网**,否则 D11 落不了地 |

「它属于谁」那个问法会**同时免掉两样**,而其中一样(盘况)免掉之后,D11 就变成一条永远无法满足的要求。

## A 案与选定形状不是一回事(Fan 问,已答)

| | 走法权威 | 盒子的角色 |
|---|---|---|
| **本文路线 A**(未被采纳) | **搬回盒上**,云端只做撮合与账本 | 权威 |
| **D8 复审选定的** | **留在云端 adapter** | **外设**(输入 + 显示) |

⇒ **这个区别对围棋要紧:围棋独立部署、不受 D8 约束,路线 A 仍然在桌上。**
对三棋不要紧 —— 三棋共享一个 `lobby_api` 进程,权威搬回盒上等于推翻 D8 本体。
