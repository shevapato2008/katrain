...
- **验收**:
  ...
  11. pytest(**并发与交付**,2026-09-24 Codex 第 1 轮后新增):重定位从占住到交付完成持有互斥,期间 `start()` 抛 `CalibrationBusy`、`confirm_existing()` 抛 `ValueError`;漂移线程拿旧 monitor 算出的结论不许把刚换上的新锁降级;`on_success` 抛错时不许停在 `ready`(降级 + `relocate_error="delivery_failed"`)。
  12. pytest:`relocate()` 在 `ready` / `required` 两态拒绝(前者会拿低精度外框锁换掉好锁,后者会绕过「沿用上次标定」)。
  13. pytest:新锁记**本次帧**的 `source_width/height`(旧锁四角先按旧源分辨率换算再对齐)、外框法的 `confidence`,`diag` 标 `relocated_by`;四角平均位移超过 3 格判「挪得太远」拒绝。失败原因放 `status()["relocate_error"]`,`metrics` 只放数值。
...
- **翻开之前还缺一样东西:识别栅栏**(2026-09-24 Codex 第 1 轮)。自动重定位期间不挂起识别(§2.1 R3),worker 仍按已知错误的旧几何识别几秒。现成的 `vision.pause_detection()` 能停住落子确认与盘面同步,但它和**终局暂停**、LED 提示共用同一套暂停机制 —— 本轮去用它,恢复时可能把「终局即停止比对」(09-20)一起放开。所以本轮不接;翻开开关的那次改动里,要先有一个**按原因计数**的暂停(或确认现有机制已是按原因的),再在重定位窗口里用它,并补「窗口内落子队列不前进」的测试。
...
| `/geometry/layout` 与俯视预览不做分辨率换算(报当前帧尺寸,却原样给锁的像素坐标;worker 那边有 `adjust_M_for_resolution`,端点没有) | 既有行为,本轮不修 | 盒上采集与识别共用一个 CameraHub,同一分辨率,实际不触发;精度工具靠「同一次开机刚标的锁」这条前置绕开(plan Task 8b)。登记在这里,别再当新发现。 |
| 盘被整个转了 90° 后的重定位 | 做不到,不做 | 外框四重对称,从外框认不出转了几个 90°;本赛道的前提是「碰动」(小位移),挪得超过 3 格就拒绝、交给重新标定。 |
