...
| 阶段 | 行为 |
|---|---|
| 第 1-2 次失败 | poller 静默自动重试（物理子被再次确认 → 重新提交同一手；隧道无状态，重发安全） |
| 达到阈值 | 暂停落子检测（防继续打隧道）→ 广播 `physical_engine_error` → 屏幕对话框 |
| 用户选「重试」 | 后端用记录的坐标重新提交；成功 → 恢复检测、正常续行；失败 → 对话框留存并显示错误 |
| 用户选「拿回棋子」 | 屏幕指引拿除该子 → 视觉确认盘面回到数字盘状态 → resync + 恢复检测（这手从未进数字盘，无需悔棋） |
| 用户选「认输」 | 走既有 resign 链路（`resign_engine_game` → game_ended → 清灯退出） |

### 3.3 悔棋（G4 修复后的语义）

- engine 局允许悔棋（星阵隧道无状态，重建 moves 序列后继续 genmove 是安全的）。
...
- R1.1 engine 局开局时给虚拟对手打「平台引擎」标记（经 `players_info` 单一信源下发，前后端共用）。
...
| `player_subtype` 语义占用 | 待核实 | R1.1 标记若与既有 subtype 消费方（rank 显示等）冲突，改用独立字段（plan Task 中核实） |
