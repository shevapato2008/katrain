...
22. **换轨瞬间在飞的同步队列项会被永久丢掉（P1 发现，非 P1 引入）。** 云端改密码后盒子
...
23. **盒子的"在线"指示对鉴权失效是瞎的（P1 发现，非 P1 引入）。** 在线判定走**不带鉴权**的
    ...
    **2026-09-14 合并 develop 之后这条的射程变大了**：develop 的 `2c2034e0`/`a979132a`
    把「闸建在 token 上而盒上 token 恒为 null」那一类死屏修好了（大厅、跨平台、复盘链，
    共 14 处），口径改成判 `isAuthenticated` 不判 token。方向完全正确，但副作用是
    **那些原本就是死的屏幕现在真的会发请求** ⇒ 云端会话死掉时，静默吞掉 401 的表面
    比 P1 终审时数到的更多。而盒端 `isAuthenticated` 认的是本机影子会话（见第 22 条），
    云端改密码打不掉它 ⇒ 屏上仍显示已登录。修这一条时要按**合并后**的落点重新数一遍。

24. **`tests/web_ui/test_social_api.py` 每跑一次就往仓库根漏一个未被 gitignore 的
...
25. **`RemoteAPIClient.heartbeat`（`remote_client.py:428-447`）在本仓零调用者。**
