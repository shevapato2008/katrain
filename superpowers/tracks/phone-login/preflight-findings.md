...
**三条路，计划里必须选一条并写死判据：**
- (a) 给容器配 `FORWARDED_ALLOW_IPS=172.20.0.1`。**已被实测排除**：两台机器的容器网关
  本来就不是同一个值 —— 生产 `010014AC` = **172.20.0.1**（网络 `katrain-ucloud_app`），
  测试 `010013AC` = **172.19.0.1**（网络 `katrain_default`），两边 `FORWARDED_ALLOW_IPS`
  都是空。写字面量意味着两套配置，而 Docker 重建网络时网段会变，
  **变了之后是静默退回"全站一个桶"，没有任何报错**。
- (b) ✅ **选这条**：应用自己解析 `X-Forwarded-For`，取**右起第 N 跳**，N 由配置给
  （`TRUSTED_PROXY_HOPS`，默认 1）。两台机器今天都恰好是 1 跳 nginx，
  同一个默认值通吃；将来前面加 CDN 只改配置不改代码。判据可单测，不依赖运行环境。
