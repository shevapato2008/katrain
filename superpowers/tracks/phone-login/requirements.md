...
6. **三个新端点每一个都要显式回答盒子问题**（F6，D-U5 之后又加了 `set-password`，
   变成四个，本约束同样覆盖它）：strict 盒子上是不是 403？
...
三个新端点（D-U5 之后又加了 `set-password`，变成四个，本约束同样覆盖它）：
strict 盒子 403（与 `auth.py:234/291` 同形）、board 非 strict 503 `need_online_phone`
**且不转发**、server 正常。每一个都要写，一个都不能漏——漏掉的那个在盒子上表现为「本地库里查无此人」，
一种查不出原因的失败（F6）。
