...
2. 检查登录页:
   - [X] 背景色为墨黑 (`#1a1714`)，不是纯黑
   - [X] "KaTrain" 标题为翡翠绿 (`#5cb57a`)
   - [X] "用户名" 和 "PIN" 输入框正常显示
   - [X] "登录" 按钮为绿色，高度较大 (56px)
   - [X] 字体为 Noto Sans SC（中文无衬线字体，非浏览器默认宋体）
3. 输入任意用户名 + PIN，点击登录:
   - [X] 成功跳转到主界面（不报错）
4. 打开 DevTools → Network:
   - [X] 字体文件从本地加载（`/@fontsource/...`），无外部 CDN 请求
...
1. StatusBar (顶部 40px):
   ...
   - [X] 左侧: "KaTrain" 文字 + 绿色引擎状态圆点
   - [X] 右侧: 用户名 + 当前时间 (HH:MM)
   - [X] 底部有分割线
2. NavigationRail (左侧 72px):

   - [X] 6 个图标+文字: 对弈、死活、研究、棋谱、直播 + 分割线 + 设置
   - [X] 当前页面对应项高亮为绿色
   - [X] 点击各项能切换页面，高亮跟随
3. 布局整体:

   - [X] StatusBar + NavigationRail + 主内容区三部分完整
   - [X] 无滚动条溢出
   - [X] 页面不闪烁
4. 路由:

   - [X] 直接访问 http://localhost:5173/kiosk/ → 跳转登录页
   - [X] 登录后 /kiosk/ → 自动跳转到 /kiosk/play
   - [X] 访问 /kiosk/nonexistent → 未登录时先跳转 /kiosk/login，登录后重定向到 /kiosk/play
   - [X] SPA 深链接: 直接访问 /kiosk/tsumego → 跳转登录页（不是 404）
5. Galaxy 未被破坏:

   - [X] http://localhost:5173/galaxy/ 正常加载，与 kiosk 无关
...
1. 对弈页 (PlayPage) 中的 ModeCard:
   ...
   - [X] 4 张卡片有图标 + 标题 + 副标题
   - [X] 鼠标点击有 scale(0.96) 按下动画
   - [X] 卡片有边框，背景色区别于页面背景
2. AI 设置页中的 OptionChips (点击"自由对弈"进入):

   - [X] 棋盘选项 (9路/13路/19路) 显示为横排按钮
   - [X] 选中项有绿色边框和深色背景
   - [X] 点击切换选中状态
   - [X] 每个 chip 最小高度 48px（触控友好）
...
- [X] 两个区块标题: "人机对弈" 和 "人人对弈"
- [X] 4 张 ModeCard: 自由对弈、升降级对弈、本地对局、在线大厅

**AiSetupPage (自由对弈 → 设置):**

- [X] 左侧棋盘占位 (土黄色，显示 "19x19")
- [X] 右侧表单: 棋盘 / 我执 / AI 强度 / 让子 / 用时
- [X] 有返回按钮
- [X] 底部 "开始对弈" 绿色大按钮
...
- [X] 点击 "开始对弈" 后进入全屏
- [X] **无** NavigationRail 和 StatusBar
- [X] 左侧棋盘占位显示 "第42手"
- [X] 右侧面板: 玩家名 (张三 / KataGo)、胜率 56.3%、AI 推荐 (R16)
- [X] 6 个控制按钮 (3x2): 悔棋 / 跳过 / 计数 / 认输 / 设置 / 退出
