...
- 🔴 **R8 真凶** AI 落子每步 ~11s 的主因 = `engine.py:_post_json` 每条查询 `multiprocessing spawn` 一个子进程,spawn 重新 import 整个 `katrain.web.server`(RK3562 ~9s);引擎本身仅 1.2s。改线程/async httpx。详见 `R8-http-engine-per-query-spawn.md`。
