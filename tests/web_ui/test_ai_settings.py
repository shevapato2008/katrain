...
async def test_estimate_rank(client):
    ...
    response = await client.post("/api/ai/estimate-rank", json={"strategy": "ai:default", "settings": {}})
    assert response.status_code == 200
    assert response.json()["rank"] == "9d"

    # Test rank estimation for scoreloss with strength adjustment
    response = await client.post(
        "/api/ai/estimate-rank", json={"strategy": "ai:scoreloss", "settings": {"strength": 0.5}}
    )
    assert response.status_code == 200
...
async def test_config_endpoints(client):
    ...
    # Update config
    response = await client.post(
        "/api/config", json={"session_id": session_id, "setting": "general/debug_level", "value": 1}
    )
    assert response.status_code == 200
